...
function requireActiveSession() {
  ...
  if (!session) {
    throw new Error("No active collaboration session. Start one with /collab <task>.");
  }
  if (session.status !== "active") {
    throw new Error(
      `Session ${session.id} is ${session.status}. Start a new one with /collab <task>.`
    );
  }
  return session;
}
